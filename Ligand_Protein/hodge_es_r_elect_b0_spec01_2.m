...
for iname =1:80
    ...
    data = pdb2mat1(strcat(indexname,'_pocket.pqr'));
    ...
    idim=2;
    nloop=100;
    file0 = fopen(strcat('./',indexname,'_ES_b0_elec_pocket_eigv_2.txt'),'w');
    file1 = fopen(strcat('./',indexname,'_ES_b0_elec_pocket_simp_2.txt'),'w');
    for eleIndex_P=1:length(Protein_ele)
        for eleIndex_L=1:length(Ligand_ele)
           ...
           %% main part, build Rips complex based on modified M matrix
            ...
            clearvars -except indexlist fileID C idim data Protein_ele Ligand_ele eleIndex_P eleIndex_L indexname nloop file0 file1 c;
